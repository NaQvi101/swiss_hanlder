...
const CustomCarousel = ({
  ...
  const handlePrev = () => {
    setActiveIndex(
      (prevIndex) => (prevIndex - 1 + sellers.length) % sellers.length
    );
  };
  ...
  const getVisibleSellers = () => {
    ...
    console.log("visibleSellers", visibleSellers);
      ...
      <div className="flex justify-center items-center overflow-hidden">
        <div className="flex transition-transform duration-300 ease-in-out">
          {getVisibleSellers().map((seller, index) => {
            ...
            return (
              <LinkContainer to={`/featured-seller/${seller._id}`}>
                <div
                  key={index}
                  className={`flex flex-col cursor-pointer items-center justify-center ${
                    displayCount === 1
                      ...
                      ? "w-60 h-72"
                      : "w-48 h-64"
                  } mx-2 bg-white rounded-lg shadow-sm transform m-3 transition-transform ${
                    isCenter ? "scale-110 shadow-2xl" : "scale-95"
                  }`}
                  style={{
